...
function getMinDateString(daysToAdd) {
    let d = new Date(); d.setDate(d.getDate() + daysToAdd);
    let year = d.getFullYear(); let month = ("0" + (d.getMonth() + 1)).slice(-2); let day = ("0" + d.getDate()).slice(-2);
...
document.getElementById('startDate').addEventListener('change', function() {
    if (this.value) {
        ...
        if (endDateVal && endDateVal < this.value) { document.getElementById('endDate').value = this.value; }
    }
});
...
// ซ่อน/แสดงข้อมูลวางบิล และดึงที่อยู่
document.querySelectorAll('.payment-radio').forEach(radio => {
    radio.addEventListener('change', function() {
        document.getElementById('billingDetailsDiv').style.display = (this.value === 'วางบิลบริษัท - เครดิต 30 วัน (เฉพาะยอดขั้นต่ำ 50,000 บาท)') ? 'block' : 'none';
    });
});
...
// ผลสรุปรายงานและ อื่นๆ
document.getElementById('rep3').addEventListener('change', function() { 
    document.getElementById('inputRepOther').disabled = !this.checked; 
});
document.querySelectorAll('.report-radio').forEach(radio => {
    radio.addEventListener('change', function() {
        if(this.id !== 'rep3') { 
            document.getElementById('inputRepOther').disabled = true; 
            document.getElementById('inputRepOther').value = ''; 
        }
        document.getElementById('summaryDetailsDiv').style.display = (this.value === 'ต้องการรายงานสรุปผลในภาพรวม 1 ฉบับ') ? 'block' : 'none';
    });
});
...
// PDPA Modal แบบทำงานสมบูรณ์ด้วย Bootstrap JS
document.addEventListener("DOMContentLoaded", function() {
    const pdpaModal = new bootstrap.Modal(document.getElementById('pdpaModal'));
    
    // กดที่ตัวหนังสือเด้ง Modal
    document.getElementById('pdpaLabel').addEventListener('click', function(e) {
        e.preventDefault(); 
        pdpaModal.show();
    });

    // กดที่ตัวติ๊กถูกให้เด้ง Modal (ไม่ให้ติ๊กถูกทันที)
    document.getElementById('pdpaCheck').addEventListener('click', function(e) {
        if(!this.checked) return; // ถ้าเอาติ๊กออก ปล่อยให้มันเอาออกได้เลย
        e.preventDefault(); // เบรคการติ๊กไว้ก่อน แล้วเปิดหน้าต่างแทน
        pdpaModal.show();
    });

    // เวลากดปุ่ม ยอมรับ ใน Modal ค่อยสั่งให้ช่องติ๊กถูก
...
document.getElementById('btnPreview').addEventListener('click', async function() {
    ...
    formData.forEach((val, key) => { 
        if(val && !key.startsWith('fmtRadio') && key !== 'deliveryReportOther') previewHtml += `<b>${key}:</b> ${val}<br>`; 
    });
    previewHtml += `</div>`;
